...
no_flux_bnd = False  # This is a boolean variable for no_flux or fixed boundary condition
...
pinn = PINN(network).build(no_flux_bnd)
...
def prediction(epochs=50,maxT=0,R0=0.5,re=1e-5,expansion_terms = 0,num_train_samples=int(2e6),initial_weights = None,train=True,saving_directory="./Data",alpha=1.0):
    """
    ...
    file_name = f'R0={R0:.2E} re={re:.2E} num_correction={num_correction} maxY={maxY:.2E} maxR={maxR:.2E} alpha={alpha:.2f} epochs={epochs:.2E} n_train={num_train_samples:.2E}'
    ...
    elif expansion_terms == 1:
        aux2_dmn0[:,0] = - script_L * (TYR_dmn0[:,1])**2 * (1.0 - f1)
    elif expansion_terms == 2:
        aux2_dmn0[:,0] = - script_L * (TYR_dmn0[:,1])**2 * (1.0 - f1 + f2)
    elif expansion_terms == 3:
        aux2_dmn0[:,0] = - script_L * (TYR_dmn0[:,1])**2 * (1.0 - f1 + f2 - f3)
    elif expansion_terms == 4:
        aux2_dmn0[:,0] = - script_L * (TYR_dmn0[:,1])**2 * (1.0 - f1 + f2 - f3 - f4)
    elif expansion_terms == 5:
        aux2_dmn0[:,0] = - script_L * (TYR_dmn0[:,1])**2 * (1.0 - f1 + f2 - f3 - f4 + f5)
    elif expansion_terms == 6:
        aux2_dmn0[:,0] = - script_L * (TYR_dmn0[:,1])**2 * (1.0 - f1 + f2 - f3 - f4 + f5 - f6)
    else:
        raise ValueError 
    ...
    if expansion_terms == 0:
        aux3_dmn0[:,0] = script_L * TYR_dmn0[:,2] * TYR_dmn0[:,1] * (1.0)
    elif expansion_terms == 1:
        aux3_dmn0[:,0] = script_L * TYR_dmn0[:,2] * TYR_dmn0[:,1] * (1.0 - g1)
    elif expansion_terms == 2:
        aux3_dmn0[:,0] = script_L * TYR_dmn0[:,2] * TYR_dmn0[:,1] * (1.0 - g1 + g2)
    elif expansion_terms == 3:
        aux3_dmn0[:,0] = script_L * TYR_dmn0[:,2] * TYR_dmn0[:,1] * (1.0 - g1 + g2 - g3)
    elif expansion_terms == 4:
        aux3_dmn0[:,0] = script_L * TYR_dmn0[:,2] * TYR_dmn0[:,1] * (1.0 - g1 + g2 - g3 - g4)
    elif expansion_terms == 5:
        aux3_dmn0[:,0] = script_L * TYR_dmn0[:,2] * TYR_dmn0[:,1] * (1.0 - g1 + g2 - g3 - g4 + g5)
    elif expansion_terms == 6:
        aux3_dmn0[:,0] = script_L * TYR_dmn0[:,2] * TYR_dmn0[:,1] * (1.0 - g1 + g2 - g3 - g4 + g5 - g6)
    else:
        raise ValueError
    ...
    for index,time_sect in enumerate(time_sects):
        ...
        ax.set_title(r'$Sc^{\frac{1}{3}}Re^{\frac{1}{2}}$'+f'={Sc**(1/3)*Re**(1/2):.5f}' + f'\n# Corrections={num_correction}')
        ...
        J_sses.append(J_ss)
        n_corrections.append(num_correction)
...
if __name__ =='__main__':
    ...
    saving_directory = f'No Flux = {no_flux_bnd} R0 = 0.05 2 correction terms'
    initial_weights = default_weight_name
    for R0 in [0.05]:
        for num_train_samples in [3e6]:
            for re in np.arange(7e-6,4.3e-5,step=2e-6):
                for num_correction in [2]:
                    initial_weights = prediction(epochs=epochs,R0=R0,re=re,alpha=0.98,expansion_terms=num_correction,num_train_samples=num_train_samples,train=False,initial_weights=initial_weights,saving_directory=saving_directory)
                    
                    df = pd.DataFrame({'X':X_s,"J_ss":J_sses,'Num Corrections':n_corrections})
