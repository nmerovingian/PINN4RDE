...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self,r1,D,R0,R1,R2,R3,Re,Sc,L,scriptL,n_correction_R,n_correction_Y,Tsim,Rsim,Ysim, num_train_samples, batch_size,):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        x_train = [TYR_dmn0,aux1overR_dmn0,auxY_dmn0,auxR_dmn0,TYR_dmn1,aux1overR_dmn1,auxY_dmn1,auxR_dmn1,TYR_bnd0,TYR_bnd1,TYR_bnd2,TYR_bnd3,TYR_bnd4,TYR_bnd5,TYR_bnd6]
...
def prediction(epochs=50,Tsim=0,freq=5,R0=0.5,r1=1e-5,r2=2e-5,r3=3e-5,lambda_ratio=3.0,n_correction_R=0,n_correction_Y=0,num_train_samples=int(2e6),batch_size = 250,initial_weights = None,train=True,saving_directory="./Data",alpha=1.0):
    """
    ...
    training_generator = DataGenerator(r1=r1,D=D,R0=R0,R1=R1,R2=R2,R3=R3,Re=Re,Sc=Sc,L=L,scriptL=scriptL,n_correction_R=n_correction_R,n_correction_Y=n_correction_Y,Tsim=Tsim,Rsim=Rsim,Ysim=Ysim,num_train_samples=num_train_samples,batch_size=batch_size)
    ...
    if train:
        ...
        history = pinn.fit(training_generator,epochs=epochs,verbose=2,callbacks=[lr_scheduler_callback], max_queue_size=10, workers=1,use_multiprocessing=False)
    ...
    for index,time_sect in enumerate(time_sects):
        ...
        ax.set_title(r'$Sc^{\frac{1}{3}}Re^{\frac{1}{2}}$'+f'={Sc**(1/3)*Re**(1/2):.5f} RadialDiffusion={RADIALDIFFUSION}' + f'\n# n_corr_R={n_correction_R} n_corr_Y={n_correction_Y}' + f"freq={freq:.1f}Hz" + f"r1={r1:.2E}m r2={r2:.2E} m r3={r3:.2E}m")

        ax = axes[1]
        ax.plot(R_flat,Flux_density)
        
        ax.set_title(f"$J_D$={J_D:.4f},$J_R$={J_R:.4f},$J_{{D,Levich}}$={J_D_Levich:.4f},$N$={collection_efficiency:.2%},$N_{{AB}}$={collection_efficiency_AB:.2%},$N_{{IL}}$={collection_efficiency_IL:.2%}")
...
if __name__ =='__main__':
    ...
    total_number_jobs = len(batch_size_sweep) * len(freq_sweep) * len(lambda_ratio_sweep) * len(radius_multiple_sweep) * len(num_train_samples_sweep)

    parameter_space = product(batch_size_sweep,freq_sweep,lambda_ratio_sweep,radius_multiple_sweep,num_train_samples_sweep)
    ...
    for TASK_ID in range(0,10):
        ...
        def train_and_save(n_correction_R,n_correction_Y):
            ...
            initial_weights = prediction(epochs=epochs,freq=freq,R0=0.05,r1=r1,r2=r2,r3=r3,lambda_ratio=lambda_ratio,alpha=0.98,n_correction_R=n_correction_R,n_correction_Y=n_correction_Y,num_train_samples=num_train_samples,batch_size=batch_size,train=False,initial_weights=initial_weights,saving_directory=saving_directory)
            sleep(randint(1,20))

            df = pd.DataFrame({"freq":freqs,'r1':r1s,"r2":r2s,"r3":r3s,'X':X_s,"J_D":J_Ds,"J_R":J_Rs,"J_D_Levichs":J_D_Levichs,'n_corr_R':n_corrections_list_R,"n_corr_Y":n_corrections_list_Y,"collection efficiency":Ns,"collection efficiencyAB":NABs,"collection efficiencyIL":NILs})
            df.to_csv(f'{saving_directory}/results.csv',index=False)
            #df.to_csv(f'{results_directory}/Results ID = {SLRUM_ARRAY_TASK_ID} epochs = {epochs} freq = {freq:.2E} lambda_ratio = {lambda_ratio:.2f} radius_multiple = {radius_multiple:.2f} n_train={num_train_samples:.2E} batch_size = {batch_size:.2E} No Flux = {NO_FLUX_BND} RadialD = {RADIALDIFFUSION}.csv',index=False)
