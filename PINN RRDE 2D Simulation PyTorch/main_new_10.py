...
def collate_fn(batch):
    # A custom collate function if your dataset produces complex or non-standard batches
    # Otherwise, you can skip this if your dataset returns tensors that can be automatically batched
    # Combine batch data returned from ConvDiffDataset.__getitem__
    # Here, we assume `x_train` and `y_train` are lists of tensors returned by each __getitem__ call
    x_train = [item[0] for item in batch]
    y_train = [item[1] for item in batch]
    # Example of handling a simple case:
    return torch.cat(x_train, dim=0), torch.cat(y_train, dim=0)


def train_model(data_loader, model, alpha, base_lr, epochs=50, ):
    # Define the lambda function for the scheduler
    lambda_lr = lambda epoch: 1.0 if epoch <= 50 else alpha ** (epoch - 50)
    optim = torch.optim.Adam(pinn.parameters(), lr=base_lr)
    # Scheduler
    scheduler = LambdaLR(optim, lr_lambda=lambda_lr)
    log_history = {"loss": [], "lr": []}
    # The learning rate scheduler
    for epoch in range(epochs):
        for x_train, y_train in tqdm(data_loader, desc=f"Training epoch {epoch}"):
            optim.zero_grad()
            y_pred = model(x_train)
            loss = 0
            for i in range(len(y_pred)):
                loss += F.mse_loss(y_pred[i].squeeze(), torch.tensor(y_train[i], dtype=torch.float32).to(device).squeeze())
            loss.backward()
            optim.step()
        print(f'Epoch {epoch}, Loss {loss.item()}')
        log_history[f"loss"].append(loss.item())
        log_history[f"lr"].append(optim.param_groups[0]['lr'])
        scheduler.step()
    return log_history


def batch_inference(data: torch.Tensor, model: nn.Module, batch_size: int = 100):
    def inference_generator(data: torch.Tensor, model: nn.Module):
        with torch.no_grad():
            for i in range(0, data.size(0), batch_size):
                yield model(data[i:i + batch_size])

    predictions = []
    data = torch.from_numpy(data).to(device).to(torch.float32)
    model.eval()
    for pred in inference_generator(data, model):
        predictions.append(pred)

    # Concatenate all predictions (if necessary)
    predictions = torch.cat(predictions, dim=0)
    return predictions.to('cpu').numpy()


def inference_with_grad(input: torch.Tensor, model: nn.Module):
    def model_apply(inp):
        return model(inp)
    input = torch.tensor(input, dtype=torch.float32).to(device)
    input.requires_grad_(True)
    model.eval()
    output = model_apply(input)
    # Calculate the Jacobian
    jacob = torch.autograd.functional.jacobian(model_apply, input)
    diagonal_jacobian = torch.zeros(input.size(0), input.size(1))
    for i in range(input.size(0)):
        diagonal_jacobian[i] = jacob[i, 0, i, :]  # Extract diagonal blocks
    return output.to('cpu').numpy(), diagonal_jacobian.to('cpu').numpy()
...
def prediction(epochs=50, Tsim=0, freq=5, R0=0.5, r1=1e-5, r2=2e-5, r3=3e-5, lambda_ratio=3.0, n_correction_R=0,
    ...
    file_name = f'maxR={Rsim} alpha={alpha} epochs={epochs} No Flux={NO_FLUX_BND} RadialD={RADIALDIFFUSION}'

    train_loader = DataGenerator(r1=r1, D=D, R0=R0, R1=R1, R2=R2, R3=R3, Re=Re, Sc=Sc, L=L, scriptL=scriptL,
    ...
    pinn.load_state_dict(torch.load(default_weight_name))
    # If train = True, fit the NN, else, use saved weights
    print(f'./weights/weights {file_name}.h5')

    need_training = True if train else False
    try:
        pinn.load_state_dict(torch.load(f'./weights/weights {file_name}.h5'))
    except:
        need_training = True
    ...
    if need_training:
        if initial_weights:
            pinn.load_state_dict(torch.load(initial_weights))
        history = train_model(train_loader, pinn, alpha, 1e-3, epochs=epochs)
        history = pd.DataFrame(history)
        history.to_csv(f'{saving_directory}/ history {file_name}.csv')
        torch.save(pinn.state_dict(), f'./weights/weights {file_name}.h5')
    ...
    for index, time_sect in enumerate(time_sects):
        ...
        C = batch_inference(XYR_test, model=network, batch_size=batch_size)
        C = C.reshape(Y.shape)
        ...
        TYR_flux[:, 2] = R_flat
        C, dC_dTYR = inference_with_grad(TYR_flux, network)
        ...
        C, dC_dTYR_disk = inference_with_grad(TYR_flux_disk, network)
        ...
        C, dC_dTYR_ring = inference_with_grad(TYR_flux_ring, network)
        ...
        fig.savefig(os.path.abspath(os.path.join(saving_directory, f'time={index}.png')), bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    RADIALDIFFUSION = True  # This Flag indicates Radial Diffusion will be incorporated into the convective-diffusion mass transport.

    # build a core network model
    network = Network()
    # build a PINN model
    pinn = PINN(network, RADIALDIFFUSION, NO_FLUX_BND).to(device)
    default_weight_name = f"./weights/default {TASK_ID}.h5"
    os.makedirs(os.path.dirname(default_weight_name), exist_ok=True)

    if not os.path.exists(results_directory):
        os.mkdir(results_directory)

    torch.save(pinn.state_dict(), default_weight_name)

    (r1s, r2s, r3s, freqs, X_s, J_Ds, J_Rs,
     J_D_Levichs, n_corrections_list_R,
     n_corrections_list_Y, Ns, NABs, NILs) = [], [], [], [], [], [], [], [], [], [], [], [], []
    ...
    # radius_multiple_sweep = [11.0,12.0,13.0,14.0,15.0,16.0,17.0,18.0,19.0,20.0]
    ...
    for TASK_ID in range(0, 10):
        ...
        def train_and_save(n_correction_R, n_correction_Y):
            ...
            saving_directory = f'Round 10 ID={TASK_ID} epochs={epochs} lambda_ratio={lambda_ratio:.2f} radius_multiple={radius_multiple:.2f} No Flux={NO_FLUX_BND} RadialD={RADIALDIFFUSION}'
